...
DATASET_PATH = os.path.join(BASE_DIR, "Censys/data/processed/dataset_features_final_v4.csv")
OUTPUT_DIR = os.path.join(BASE_DIR, "Results/Results_dataset_v4")
os.makedirs(OUTPUT_DIR, exist_ok=True)

np.random.seed(42)

print("=" * 70)
print("PHISHING DETECTION - LOGISTIC REGRESSION PIPELINE (V4)")
print("=" * 70)
...
print("[0/9] Dataset overview...")
print(f"      Samples: {len(df)}")
print(f"      Raw features: {len(X.columns)}")
print(f"      Numeric: {len(num_cols)} | Categorical: {len(cat_cols)}")
print(f"      Positives (phishing): {(y == 1).sum()} | Negatives (legit): {(y == 0).sum()}")
...
# =========================
# Lock OHE categories using TRAIN set
...
plt.title("ROC Curve - Phishing Detection (Dataset v4)", fontsize=14, fontweight="bold")
...
plt.title("Precision-Recall Curve - Phishing Detection (Dataset v4)", fontsize=14, fontweight="bold")
...
with open(summary_path, "w") as f:
    f.write("=" * 80 + "\n")
    f.write("FEATURE ANALYSIS SUMMARY - LOGISTIC REGRESSION (V4)\n")
...
print(f"[9/9] Done. Results saved in: {OUTPUT_DIR}")
